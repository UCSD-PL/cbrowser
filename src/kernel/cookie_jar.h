...
struct cookie_list {
  int FINAL REF(V > 0) num_cookies;
  struct cookie FINAL * FINAL * FINAL CookieList(num_cookies) cookies;
};

void
add_cookie(int soc,
           struct cookie FINAL * CookiePtr REF(TAGSET([V]) = Set_sng([soc]))
...
NNREF(? COOKIE_DOMAIN_GET([THE_STRING([domain]); DOMAIN([V])]))
