...
gettable_domains(char FINAL NULLTERMSTR ICHAR * STRINGPTR REF(DOMAIN([V]) = THE_STRING([V])) d) OKEXTERN;
...
/* parse_cookie(const char NULLTERMSTR ICHAR FINAL * STRINGPTR cookie_str, size_t n) OKEXTERN; */


/* void */
/* print_cookie(struct cookie *c) OKEXTERN; */

/* char NULLTERMSTR ICHAR * START STRINGPTR REF(DOMAIN([V]) = DOMAIN([DEREF([s])])) */
...
/* read_is_transitive(char FINAL NULLTERMSTR ICHAR * STRINGPTR s1, */
/*                    char FINAL NULLTERMSTR ICHAR *  */
